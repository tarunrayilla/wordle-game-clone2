document.addEventListener("DOMContentLoaded", () => {
    ...
    const keys = document.querySelectorAll(".keypad-row button");

    function getCurrWordArr() {
        return wordsTried[wordsTried.length - 1]
    }
    ...
    function handleSubmitWord() {
        ...
        const firstLetterId = wordsTriedCount * 5 + 1;
        const interval = 200;
        currWordArr.forEach((letter, index) => {
          setTimeout(() => {
            const tileColor = getSquareColor(letter, index);
    ...
    function handleDeleteLetter() {
        ...
        const lastLetterEl = document.getElementById(String(currSquare - 1));
    
        lastLetterEl.textContent = "";
        currSquare = currSquare - 1;
      }

    function updateWordsTried(letter) {
        const currWordArr = getCurrWordArr()

        if(currWordArr && currWordArr.length < 5) {
            currWordArr.push(letter)

            const currSquare1 = document.getElementById(String(currSquare));
            currSquare = currSquare + 1;
            currSquare1.textContent = letter;
